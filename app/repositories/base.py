...
class BaseRepository:
    ...
    def set_filters(self, query, kwargs: dict[str, Any]):
        filters = []
    ...
    async def get(self, session: AsyncSession, **kwargs) -> Union[TypeModel, None]:
    ...
    async def get_or_create(self, instance: TypeModel, session: AsyncSession) -> TypeModel:
        ...
        got_instance = await self.get(session=session, **instance_dict)
        if got_instance:
            return got_instance
        return await self.create(instance=instance, session=session)

    async def all(self, session: AsyncSession) -> Union[Iterable[TypeModel], None]:
        query = select(self.model)
        result = await session.execute(query)
        return result.scalars().all() if result else None

    async def filter(self, session: AsyncSession, order_by: str = None, **kwargs) -> (
            Union)[Iterable[TypeModel], None]:
        query = select(self.model)
        if kwargs:
            query = self.set_filters(query=query, kwargs=kwargs)
        if order_by:
